...
# %% 7) Write data for MESSAGE (CF)

# [export to Excel] flow time series of capacity factors with 8760 time slices (pseudo-hourly)
writer_slices : ExcelWriter = pd.ExcelWriter('Results_8760_slices_CF.xlsx')
df_names_slices = np.transpose(df_names)
df_countries_slices = np.transpose(df_countries)

# [write] plant and country names
df_names_slices.to_excel(writer_slices, 'Results_normal', index = False, startrow = 0, header = False)
df_countries_slices.to_excel(writer_slices, 'Results_normal', index = False, startrow = 1, header = False)
df_names_slices.to_excel(writer_slices, 'Results_dry', index = False, startrow = 0, header = False)
df_countries_slices.to_excel(writer_slices, 'Results_dry', index = False, startrow = 1, header = False)
df_names_slices.to_excel(writer_slices, 'Results_wet', index = False, startrow = 0, header = False)
df_countries_slices.to_excel(writer_slices, 'Results_wet', index = False, startrow = 1, header = False)
    
# [write] monthly CF to hourly CF
CF_bymonth_normal_slices = np.zeros(shape = (np.sum(days_year)*hrs_day, len(index)))
CF_bymonth_dry_slices = np.zeros(shape = (np.sum(days_year)*hrs_day, len(index)))
CF_bymonth_wet_slices = np.zeros(shape = (np.sum(days_year)*hrs_day, len(index)))
for n in range(len(index)):
    for m in range(months_yr):
        CF_bymonth_normal_slices[int(positions[m]):int(positions[m+1]), n] = CF_bymonth_normal[n,m]
        CF_bymonth_dry_slices[int(positions[m]):int(positions[m+1]), n] = CF_bymonth_dry[n,m]
        CF_bymonth_wet_slices[int(positions[m]):int(positions[m+1]), n] = CF_bymonth_wet[n,m]

# [write] output on each sheet
df_output_slices_normal = pd.DataFrame(CF_bymonth_normal_slices[:, output_binary])
df_output_slices_normal.to_excel(writer_slices, 'Results_normal', index = False, startrow = 2, header = False)

df_output_slices_dry = pd.DataFrame(CF_bymonth_dry_slices[:, output_binary])
df_output_slices_dry.to_excel(writer_slices, 'Results_dry', index = False, startrow = 2, header = False)

df_output_slices_wet = pd.DataFrame(CF_bymonth_wet_slices[:, output_binary])
df_output_slices_wet.to_excel(writer_slices, 'Results_wet', index = False, startrow = 2, header = False)

writer_slices.save()


print('Results for Excel ready (II)')


# %% 8) Write data for MESSAGE (inflow)


# [export to Excel] flow time series of bias-corrected inflows with 8760 time slices (pseudo-hourly)
writer_slices : ExcelWriter = pd.ExcelWriter('Results_8760_slices_inflow.xlsx')
df_names_slices = np.transpose(df_names)
df_countries_slices = np.transpose(df_countries)

# [write] plant and country names
df_names_slices.to_excel(writer_slices, 'Results_normal (m^3s^-1)', index = False, startrow = 0, header = False)
df_countries_slices.to_excel(writer_slices, 'Results_normal (m^3s^-1)', index = False, startrow = 1, header = False)
df_names_slices.to_excel(writer_slices, 'Results_dry (m^3s^-1)', index = False, startrow = 0, header = False)
df_countries_slices.to_excel(writer_slices, 'Results_dry (m^3s^-1)', index = False, startrow = 1, header = False)
df_names_slices.to_excel(writer_slices, 'Results_wet (m^3s^-1)', index = False, startrow = 0, header = False)
df_countries_slices.to_excel(writer_slices, 'Results_wet (m^3s^-1)', index = False, startrow = 1, header = False)
    
# [write] monthly CF to hourly CF
Q_in_bymonth_normal_slices = np.zeros(shape = (np.sum(days_year)*hrs_day, len(index)))
Q_in_bymonth_dry_slices = np.zeros(shape = (np.sum(days_year)*hrs_day, len(index)))
Q_in_bymonth_wet_slices = np.zeros(shape = (np.sum(days_year)*hrs_day, len(index)))
for n in range(len(index)):
    for m in range(months_yr):
        Q_in_bymonth_normal_slices[int(positions[m]):int(positions[m+1]), n] = Q_bias_corrected_bymonth_normal_natural[n,m]
        Q_in_bymonth_dry_slices[int(positions[m]):int(positions[m+1]), n] = Q_bias_corrected_bymonth_dry_natural[n,m]
        Q_in_bymonth_wet_slices[int(positions[m]):int(positions[m+1]), n] = Q_bias_corrected_bymonth_wet_natural[n,m]

# [write] output on each sheet
df_output_slices_normal = pd.DataFrame(Q_in_bymonth_normal_slices[:, output_binary])
df_output_slices_normal.to_excel(writer_slices, 'Results_normal (m^3s^-1)', index = False, startrow = 2, header = False)

df_output_slices_dry = pd.DataFrame(Q_in_bymonth_dry_slices[:, output_binary])
df_output_slices_dry.to_excel(writer_slices, 'Results_dry (m^3s^-1)', index = False, startrow = 2, header = False)

df_output_slices_wet = pd.DataFrame(Q_in_bymonth_wet_slices[:, output_binary])
df_output_slices_wet.to_excel(writer_slices, 'Results_wet (m^3s^-1)', index = False, startrow = 2, header = False)

writer_slices.save()

print('Results for Excel ready (III)')
